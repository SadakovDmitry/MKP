const imgMkrIcons0003Layer4 = "/assets/392e3857209dc66690ca73b8a690e23d914c2f6f.png";
const imgMkrIcons0002Layer3 = "/assets/bec0c030b20d45265db78ce720b9e7821571d36a.png";
const imgMkrIcons0004Layer0Copy = "/assets/967e1e665c88e5a01a218fecde6762cf105f578c.png";
const imgMkrIcons0001Layer2 = "/assets/1a01377457f84ffc40bea44a61f20a55bf14abd5.png";
const imgMkrIcons0000Layer1 = "/assets/a47f2e952395700cbc7d73f619fb126302fe4876.png";
const imgExclude = "/assets/5b1e24b337d2c82bcf513937d48509498b8d98da.svg";

export default function Mobile() {
  return (
    <div className="bg-[var(--color,#1f556b)] relative size-full" data-name="Mobile" data-node-id="83:1027">
      <p className="-translate-x-1/2 absolute font-['Geologica:Medium',sans-serif] font-medium leading-[1.1] left-[calc(50%-0.5px)] not-italic text-[60.78px] text-[color:var(--color-4,white)] text-center top-[calc(50%-851px)] uppercase w-[539px] whitespace-pre-wrap" data-node-id="83:1028" style={{ fontVariationSettings: "\'CRSV\' 0, \'SHRP\' 0" }}>
        преимущества
      </p>
      <div className="absolute bg-[var(--color-2,#44b1d2)] h-[141.875px] left-[175px] overflow-clip rounded-[24.674px] top-[389px] w-[135.089px]" data-node-id="83:1029">
        <div className="absolute h-[73.45px] left-[27.77px] top-[34.54px] w-[78.908px]" data-name="MKR-icons_0003_Layer-4" data-node-id="83:1030">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgMkrIcons0003Layer4} />
        </div>
      </div>
      <div className="absolute bg-[var(--color-2,#44b1d2)] h-[141.875px] left-[175px] overflow-clip rounded-[24.674px] top-[573.17px] w-[135.089px]" data-node-id="83:1031">
        <div className="absolute h-[66.058px] left-[28.38px] top-[38.24px] w-[78.274px]" data-name="MKR-icons_0002_Layer-3" data-node-id="83:1032">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgMkrIcons0002Layer3} />
        </div>
      </div>
      <div className="absolute bg-[var(--color-2,#44b1d2)] h-[141.875px] left-[175px] overflow-clip rounded-[24.674px] top-[757.35px] w-[135.706px]" data-node-id="83:1033">
        <div className="absolute h-[73.92px] left-[32.08px] top-[33.92px] w-[71.653px]" data-name="MKR-icons_0004_Layer-0-copy" data-node-id="83:1034">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgMkrIcons0004Layer0Copy} />
        </div>
      </div>
      <div className="absolute bg-[var(--color-2,#44b1d2)] h-[141.875px] left-[175px] overflow-clip rounded-[24.674px] top-[941.39px] w-[135.089px]" data-node-id="83:1035">
        <div className="absolute h-[76.738px] left-[32.7px] top-[32.7px] w-[69.479px]" data-name="MKR-icons_0001_Layer-2" data-node-id="83:1036">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgMkrIcons0001Layer2} />
        </div>
      </div>
      <div className="absolute bg-[var(--color-2,#44b1d2)] h-[141.875px] left-[175px] overflow-clip rounded-[24.674px] top-[1125.56px] w-[135.089px]" data-node-id="83:1037">
        <div className="absolute h-[82.184px] left-[26.53px] top-[30.23px] w-[81.898px]" data-name="MKR-icons_0000_Layer-1" data-node-id="83:1038">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none size-full" src={imgMkrIcons0000Layer1} />
        </div>
      </div>
      <div className="absolute font-['Roboto:Regular',sans-serif] font-normal inset-[19.57%_26.87%_75.24%_31.46%] leading-none text-[26.616px] text-[color:var(--color-4,white)] whitespace-pre-wrap" data-node-id="83:1039" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        <p className="mb-0">Только проверенные решения</p>
        <p>с гарантированным результатом — каждое из них мы многократно применяли на практике.</p>
      </div>
      <p className="absolute font-['Roboto:Regular',sans-serif] font-normal inset-[27.82%_30.39%_65.69%_31.4%] leading-none text-[26.616px] text-[color:var(--color-4,white)] whitespace-pre-wrap" data-node-id="83:1040" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Консультации для малого, среднего и крупного бизнеса любой формы собственности — мы всегда на связи и готовы оказать экспертную поддержку.
      </p>
      <p className="absolute bottom-[47.98%] font-['Roboto:Regular',sans-serif] font-normal leading-none right-[740.21px] text-[26.616px] text-[color:var(--color-4,white)] top-[45.53%] translate-x-full w-[530.362px] whitespace-pre-wrap" data-node-id="83:1041" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Команда специалистов высокой квалификации с опытом работы в вашей отрасли (строительство, торговля, услуги, сельское хозяйство, переработка, промышленное производство и др.).
      </p>
      <p className="absolute font-['Roboto:Regular',sans-serif] font-normal inset-[54.45%_16.48%_39.06%_31.46%] leading-none text-[26.616px] text-[color:var(--color-4,white)] whitespace-pre-wrap" data-node-id="83:1042" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Решаем широкий спектр задач:
        <br aria-hidden="true" />
        от оптимизации учёта и бизнес-процессов до разработки организационно-финансовых схем реализации проектов и создания единого информационного пространства.
      </p>
      <p className="absolute font-['Roboto:Regular',sans-serif] font-normal inset-[37.28%_25.2%_57.53%_31.46%] leading-none text-[26.616px] text-[color:var(--color-4,white)] whitespace-pre-wrap" data-node-id="83:1043" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Сопровождаем любые коммерческие проекты от стартапа до выхода на региональный, федеральный или мировой уровень.
      </p>
      <p className="-translate-x-1/2 absolute font-['Geologica:Medium',sans-serif] font-medium leading-none left-[calc(50%-1px)] not-italic text-[50.081px] text-[color:var(--color-4,white)] text-center top-[calc(50%+360px)] w-[900px] min-h-[56px] h-auto" data-node-id="83:1044" style={{ fontVariationSettings: "\'CRSV\' 0, \'SHRP\' 0" }}>
        Мы любим то, что делаем!
      </p>
      <p className="-translate-x-1/2 absolute bottom-[21.73%] font-['Roboto:Regular',sans-serif] font-normal leading-none left-[calc(50%-1px)] text-[34.15px] text-[color:var(--color-4,white)] text-center top-[73.37%] w-[662px] whitespace-pre-wrap" data-node-id="83:1045" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Обратитесь к нам — и получите экспертную консультацию по любому вопросу или проблеме вашего бизнеса.
      </p>
      <p className="-translate-x-1/2 absolute bottom-[14.81%] font-['Roboto:Bold',sans-serif] font-bold leading-none left-[calc(50%-1px)] text-[34.15px] text-[color:var(--color-4,white)] text-center top-[81.92%] uppercase w-[588px] whitespace-pre-wrap" data-node-id="83:1046" style={{ fontVariationSettings: "\'wdth\' 100" }}>
        Для нас нет проблем — у нас есть оптимальные решения.
      </p>
      <div className="-translate-x-1/2 absolute h-[83.333px] left-[calc(50%-0.5px)] top-[1847.66px] w-[535px]" data-node-id="83:1047">
        <div className="absolute bg-[var(--color-4,white)] inset-0 rounded-[83.333px]" data-node-id="83:1048" />
        <p className="absolute font-['Geologica:Light',sans-serif] font-light inset-[29.56%_43.99%_29.64%_7.79%] leading-[0.9] not-italic text-[37.369px] text-[color:var(--color,#1f556b)] tracking-[0.7474px]" data-node-id="83:1049" style={{ fontVariationSettings: "\'CRSV\' 0, \'SHRP\' 0" }}>
          Написать нам
        </p>
        <div className="absolute inset-[10.01%_1.69%_10.83%_85.98%]" data-name="Exclude" data-node-id="83:1050">
          <img alt="" className="absolute block max-w-none size-full" src={imgExclude} />
        </div>
      </div>
    </div>
  );
}